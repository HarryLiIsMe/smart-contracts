...
contract Signature {
    function verify(address _signer,address _to,uint256 _amount,uint256 _nonce,string memory _message,uint8 v,bytes32 r,bytes32 s) external pure returns (bool) {
        bytes32 messageHash = getMessageHash(_to, _amount,_nonce, _message);
        
        bytes32 messageDigest = getEthSignedMessageHash(messageHash);
        // vrs 合并和单独发都行
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(_to, _amount,_nonce, _message));
    }
